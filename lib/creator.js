...
const extractAll = async function (sdkDocSetRoot, config, destination, template) {
  ...
  for (var i in snippets) {
    ...
    if (snippet.name) {
      ...
      if (methodList.indexOf(snippet.name) < 0) {
        methodList.push(snippet.name)
      }
    }
  }
  ...
  const backList = methodList.slice()
  // 找到余下的 case 最匹配的组
  for (var i in methodList) {
    const name = methodList[i]

    var mostMatchedPipe
    var mostMatchedMethod
    var logestSubStringMethodName = ''

    for (var j in pipelines) {
      const pipe = pipelines[j]
      for (var i in pipe.methodNames) {
        const m = pipe.methodNames[i]
        if (name.startsWith(m.name) && m.name.length > 
          logestSubStringMethodName.length) {
            mostMatchedPipe = pipe
            mostMatchedMethod = pipe.methodNames[i]
            logestSubStringMethodName = m.name
        }
      }
    }
    if (logestSubStringMethodName && mostMatchedPipe) {
      console.warn('Hyper: found snippet ', name, ' for ', logestSubStringMethodName)
      mostMatchedPipe.methods.push(Object.assign({}, mostMatchedMethod, {
        snippet: snippetNameDic[name],
        name: name
      }))
      const idx = backList.indexOf(name)
      if (idx >= 0) {
  ...
  for (var i in backList) {
    console.warn('Warn: discard ', backList[i])
  }

  for (var i in pipelines) {
    const pipeline = pipelines[i]
    if (pipeline.methods.length < 1) {
      console.warn('Warn: discard group for empty case:', pipeline.name)
      continue
    }
    // 生成示例文件
