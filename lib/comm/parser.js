...
class SegmentNode {
  constructor(level, title) {
    ...
    // 代码段开始结束符号```在节点内容中的偏移
    this.codeIndexes = []
    // 代码段标签 [//]: # (.cssg-snippet-xxx) 在节点内容中的偏移
    this.tagIndexes = []
    // 代码段标签名称
    this.tagName = ''
  }
...
const treeify = function(docContent) {
  const root = SegmentNode(0, null)

  var regex1 = RegExp('(#{1,5})\\s+([^`(\\r|\\n||\\r\\n)]+)(\\r|\\n||\\r\\n)')
  var aboveNode = root // 上一个结点
  var array1;
  while ((array1 = regex1.exec(docContent)) !== null) {
    const level = array1[1]
    const title = array1[2]
    const node = SegmentNode(level, title)
    node.lineBreaker = array1[3]
    // 开始
    ...
    aboveNode.content = docContent.substring(aboveNode.indexes[0], aboveNode.indexes[1])

    // 处理文档结点
    var regex2 = RegExp('\\[//\\]\\s*:\\s*#\\s*\\(\\.cssg-snippet-([^)]+)\\)','g')
    var array2;
    while ((array2 = regex2.exec(aboveNode.content)) !== null) {
      aboveNode.tagName = array2[1]
      aboveNode.tagIndexes = [regex2.lastIndex - array2[0].length, regex2.lastIndex]
    }
    if (aboveNode.tagName) {
      regex2 = RegExp('```[^`(\\r|\\n||\\r\\n)]*(\\r|\\n||\\r\\n)','g');
      while ((array2 = regex2.exec(aboveNode.content)) !== null) {
        aboveNode.codeIndexes.push(regex2.lastIndex - array2[0].length)
      }
    }
  ...
  aboveNode.content = docContent.substring(aboveNode.indexes[0])
...
const recursiveWrite = function(node, buffer, lang, getSnippetContent, 
  getRealWebUrl) {
    ...
    if (content.tagName) {
      // 文档节点
      snippet = getSnippetContent(content.tagName)
      if (!snippet) {
        // 找不到代码块，丢弃当前节点跟子节点
        return
      }
    ...
    for (const i in tagIndexes) {
      ...
      var mdContent = docContent.substring(start, codeStart)
      
  
      buffer.push(mdContent)
      // 插入```[lang]
      buffer.push('```' + lang + lineBreaker)
    ...
    if (start < docContent.length) {
      // 替换url为真实url
      regex1 = RegExp('\\(cssg://(.+)\\)','g');
      var mdContent = docContent.substring(start)
      while ((array1 = regex1.exec(mdContent)) !== null) {
        const realUrl = getRealWebUrl(array1[1])
        if (realUrl) {
          mdContent = mdContent.replace(array1[0], '(' + realUrl + ')')
        }
      }
      buffer.push(mdContent)
    }
