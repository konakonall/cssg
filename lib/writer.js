...
const write = async function (docSetRoot, projRoot, lang) {
  ...
  await writeDocuments(docTemplateRoot, caseRoot, docSetRoot, config.sourceExtension.substring(1), config)

  console.log('CSSG write document end:\n----------------- ')

}

const writeDocuments = async function (docTemplateRoot, caseRoot, docSetRoot, lang, config) {
  ...
  const changes = []
  for (var i in docs) {
    const newDoc = await parser.injectCodeSnippet2Doc(docs[i], lang, function (snippetName) {
      ...
      if (cssgUrl == 'api-doc') {
        ...
        if (snippetMap[name]) {
          return config.repoUrl + '/tree/master/' + config.caseRelativePath 
            + '/' + snippetMap[name].group
        }
      }
      return null
    })
...
const docRootSet = "/Users/laiwenjie/Workspace/qcloud-documents/product/存储与CDN/对象存储 4.0/SDK文档/Android SDK"
const lang = "android"
const projRoot = "/Users/laiwenjie/Workspace/qcloud-sdk-android/"
