...
const writeDocuments = async function (docTemplateRoot, 
  ...
  // 准备本地代码仓库
  const localRepo = await cssg.prepareLocalLanguageRepo(lang, globalConfig.snippetRepoUrl)
  ...
  for (var i in docs) {
    const newDoc = await parser.injectCodeSnippet2Doc(docs[i], config.snippetLabel, 
      ...
      if (cssgUrl == 'api-doc') {
        ...
        if (snippetMap[name]) {
          return globalConfig.snippetRepoUrl + path.join('/tree/master/', config.projRoot, 
...
const syncSnippet = async function(docSetRoot, lang) {

  console.log('CSSG write Snippets start:\n----------------- ')
  ...
  const documentRelativePath = config.documentRelativePath

  const targetDocSetDir = path.join(docSetRoot, documentRelativePath)

  const docs = util.traverseFiles(targetDocSetDir, "md")
  for (var i in docs) {
    const newDoc = await parser.injectCodeSnippet2DocOld(docs[i],
      function (snippetName) {
        if (snippetMap.hasOwnProperty(snippetName)) {
          return snippetMap[snippetName].bodyBlock
        }
