...
const langConfigUrl = 'https://raw.githubusercontent.com/konakonall/cssg/master/conf/'
...
const langConfig = async function(lang) {
  ...
  const url = `${langConfigUrl}${lang}.json`
  console.log(`CSSG fetch config from ${url}...`)

  try {
    return await fetchRemoteJSONConfig(url)
  } catch(e) {
    console.error(e)
    return null
  }
...
const prepareLocalLanguageRepo = async function(lang, remoteRepoUrl) {
  const localConf = getLocalConf();
  if (localConf != null && localConf['snippets_home']) {
    const snippetsHome = localConf['snippets_home']
    console.log(`CSSG find snippets repo: `, snippetsHome)
    return {
      home: snippetsHome,
      temp: false
    }
